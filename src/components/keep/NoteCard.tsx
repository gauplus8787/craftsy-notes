  ...
  return (
    ...
    >
      ...
      >
        <Pin
          className={`w-4 h-4 ${
            note.pinned ? "text-foreground fill-foreground" : "text-keep-icon"
          }`}
        />
      </button>
      ...
      <div className="flex items-center gap-0.5 px-1.5 py-1 opacity-0 group-hover:opacity-100 transition-opacity" onClick={(e) => e.stopPropagation()}>
        ...
        <div ref={moreRef} className="relative">
          ...
          {showMore && (
            <div className="absolute bottom-full right-0 mb-1 bg-card rounded-lg keep-shadow z-10 py-1 min-w-[160px]">
              <button
                onClick={() => {
                  onDelete(note.id);
                  setShowMore(false);
